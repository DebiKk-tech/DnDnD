...
HELLTOWER_ARE_YOU_SURE = 'Вы подходите к Адской Башне. Она пульсирует силой, исходящей из неё. Войдя внутрь, вы' \
                         ' сразитесь с одним из трёх боссов, что держат в страхе всю округу. После начала боя вы смож' \
                         'ете попытаться сбежать, но ваши шансы довольно малы. Вы уверены, что хотите войти?'
...
YOU_CHOOSE_DEATH = 'Вы вошли в Адскую Башню..'
ALL_BOSSES_DEAD = '..однако вы обнаружили, что все боссы мертвы. Однако вы можете их воскресить. Желаете?'
BOSSES_REVIVED = 'Адская Башня снова наполнилось силой, вселяя ужас в местных жителей. Боссы были воскрешены'
...
GET_BOSSES_ALIVE_QUERY = 'SELECT dead FROM bosses'
GET_BOSS_BY_ID_QUERY = 'SELECT * FROM bosses WHERE id = ?'
UPDATE_BOSS_QUERY = """UPDATE bosses
                 SET health = ?, dead = ?
                 WHERE id LIKE ?"""
GET_BOSS_ID_BY_NAME_QUERY = 'SELECT id FROM bosses WHERE name LIKE ?'
SET_GNOLL_ALIVE_QUERY = """UPDATE bosses
                            SET health = 10000
                            WHERE id = 2"""
SET_EYE_ALIVE_QUERY = """UPDATE bosses
                            SET health = 8000
                            WHERE id = 3"""
SET_ROBOT_ALIVE_QUERY = """UPDATE bosses
                            SET health = 7000
                            WHERE id = 1"""
SET_BOSSES_ALIVE_QUERY = """UPDATE bosses
                            SET dead = 0
                            WHERE name NOT LIKE ''"""
